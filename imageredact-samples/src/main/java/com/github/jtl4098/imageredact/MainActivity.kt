...
import com.github.jtl4098.imageredact.core.BitmapIO
import java.io.File
...
class MainActivity : AppCompatActivity() {
    ...
    private val imageRedact by lazy {
        ImageRedact(this, MaskOptions(debugOutline = BuildConfig.DEBUG), workerExecutor)
    }
    ...
    private fun processImageFile(photoFile: File, deleteInputAfter: Boolean) {
        imageRedact.maskFile(
            photoFile,
            onSuccess = { result ->
                val outputFile = File.createTempFile("ImageRedact_masked_", ".jpg", cacheDir)
    ...
    private fun copyUriToCache(uri: Uri): File? {
        ...
        val tempFile = File.createTempFile("ImageRedact_import_", ".jpg", cacheDir)
        try {
